...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	...
	//---------------------------------------------------------------------------------------------------------------------------------------------
    NSString *AgentUser1=user1[@"name"];
    NSString *AgentUser2=user2[@"name"];
    
    
	CreateMessageItem(user1, roomId,AgentUser2,user2);
	CreateMessageItem(user2, roomId,AgentUser1,user2);
	//---------------------------------------------------------------------------------------------------------------------------------------------
	return roomId;
}
	...
	[query findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error)
	{
        if (error == nil)
        {
            if ([objects count] == 0)
            {
                ...
                {
                     message[PF_MESSAGES_DESCRIPTION] = @"No Name";
                }else
                {
                ...
                PFUser *currentUsr=[PFUser currentUser];

                NSLog(@"description %@",description);
                NSLog(@"currentUsr.objectId %@",currentUsr.objectId);
